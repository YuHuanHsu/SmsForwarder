...
class TelegramFragment : BaseFragment<FragmentSendersTelegramBinding?>(), View.OnClickListener, CompoundButton.OnCheckedChangeListener {
        ...
        Core.sender.get(senderId).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(object : SingleObserver<Sender> {
            ...
            override fun onSuccess(sender: Sender) {
                ...
                if (settingVo != null) {
                    ...
                    binding!!.rgChatType.check(settingVo.getChatTypeCheckId())
    ...
    override fun initListeners() {
        ...
        binding!!.rgChatType.setOnCheckedChangeListener { _: RadioGroup?, checkedId: Int ->
            updateThreadIdVisibility()
        }
     ...
     */
    private fun updateThreadIdVisibility() {
        val isForumSelected = binding!!.rgChatType.checkedRadioButtonId == R.id.rb_chat_type_forum
        // 使用 View.VISIBLE 或 View.GONE 來控制顯示/隱藏
        val messageThreadIdLayout = binding!!.etMessageThreadId.parent as View
        messageThreadIdLayout.visibility = if (isForumSelected) View.VISIBLE else View.GONE
        
        // 如果不是 forum 類型，清空主題 ID
        if (!isForumSelected) {
            binding!!.etMessageThreadId.setText("")
        }
    }
    ...
    private fun checkSetting(): TelegramSetting {
        ...
        // 獲取新增的聊天類型和主題 ID 參數
        val chatType = when (binding!!.rgChatType.checkedRadioButtonId) {
            R.id.rb_chat_type_group -> "group"
            R.id.rb_chat_type_forum -> "forum"
            R.id.rb_chat_type_channel -> "channel"
            else -> "chat" // 預設值，保持向後相容
        }
        
        val messageThreadId = if (chatType == "forum") {
            val threadId = binding!!.etMessageThreadId.text.toString().trim()
            if (threadId.isEmpty()) {
                throw Exception(getString(R.string.telegram_thread_id_required))
            }
            // 驗證 messageThreadId 是否為純數字
            if (!threadId.matches(Regex("\\d+"))) {
                throw Exception(getString(R.string.telegram_thread_id_invalid_format))
            }
            threadId
        } else null

        return TelegramSetting(method, apiToken, chatId, messageThreadId, chatType, proxyType, proxyHost, proxyPort, proxyAuthenticator, proxyUsername, proxyPassword, parseMode)
    }
